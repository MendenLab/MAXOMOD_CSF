...
```{r load abundancy data}
#load expression data
      protein_expression <- read.delim("data/proteinGroups_CSF_08.2023.txt") #the txt file with the proteomic data
      
      #change colnames and select only expression values
      Intensity_names = colnames(protein_expression)[grep("Intensity.", colnames(protein_expression))] #make a vector with the column names of the actual protein abundancies
      abu_data = protein_expression[,c("Protein.IDs","Majority.protein.IDs", "id", "Peptide.IDs", "Potential.contaminant", Intensity_names)] #select the columns with IDs, potential contaminants, and protein abundancies
      colnames(abu_data) = c("name","ID", "id_1", "id_2", "Potential_contaminant", Intensity_names) #rename the selected columns
      
      #function to get correct patient ID from the LFQ names
      f = function(x){ 
        a = unlist(strsplit(x, split='.', fixed=TRUE))[2]
        if(length(unlist(strsplit(x, split='.', fixed=TRUE)))>2){
          b = unlist(strsplit(x, split='.', fixed=TRUE))[3]
          a = paste0(a," ",b)}
        return(a)}
      
      #apply the function to the column names that contain protein abundancy
      tube_id_abu_data = unlist(lapply(Intensity_names, FUN = f))
      tube_id_abu_data[tube_id_abu_data == "ALS54 CTRL54"] = "ALS54-CTRL54" #small adjustment to align tube id and clinical id
      
      #remove potential contaminants
      nrow(abu_data) #number of proteins before removing potential contaminants
      abu_data = data.table(abu_data[abu_data$Potential_contaminant != "+", ])
      #abu_data = abu_data %>% select(-Potential_contaminant)
      abu_data[, Potential_contaminant := NULL] 
      abu_data = as.data.frame(abu_data)
      #and there are no reverse proteins
      nrow(abu_data) #number of proteins after removing potential contaminants
      
      #change rownames and only select proteins with the word "HUMAN" in it
      abu_data = abu_data[grep("HUMAN", abu_data$ID),]
      nrow(abu_data) #number of proteins after removing non-human proteins
      
      #make a vector with protein names
      protein_names = abu_data$ID
      
      #make all zeroes NA
      abu_data[abu_data == 0] <- NA
```

## Change the gene name annotation using the AnnotationDbi

```{r change the gene name annotation using the AnnotationDbi}
#make a better list of Uniprot accession numbers and their gene names
      f = function(x){ #this function takes the uniprot accession number that is in the protein name
        ...
        return(c(a,b))}
      uniprot_to_genename = matrix(nrow = length(protein_names), ncol = 3) #make an empty matrix to store results
      for(i in 1:length(protein_names)){ #use the function in a loop to fill the matrix with results
        uniprot_to_genename[i,] = c(protein_names[i], f(protein_names[i]))
      }
      colnames(uniprot_to_genename) = c("full_name", "gene_name", "uniprot_accession") #change column names for the uniprot accession numbers
      uniprot_to_genename = as.data.frame(uniprot_to_genename) #change from matrix to data.frame format
      abu_data$ID = uniprot_to_genename$uniprot_accession #use the uniprot accession number as IDs in the abundancy table
      abu_data$name = uniprot_to_genename$full_name #use the full name as the 'name' in the abundancy table
      abu_data = abu_data %>% distinct(abu_data$ID, .keep_all = TRUE) #only keep unique rows according to their uniprot accession number --> it removes 1 protein
      

      #use uniprot to look up best annotation of the gene name instead of using the one provided in the dataset --> this results in more accurate annotation that causes other packages to work better
      genes = AnnotationDbi::select(org.Hs.eg.db, uniprot_to_genename$uniprot_accession, "SYMBOL", "UNIPROT") #use AnnotationDbi database to retrieve the annotations
      genes = genes %>% distinct(genes$UNIPROT, .keep_all = TRUE) #keep only unique uniprot accession numbers
      uniprot_to_genename = uniprot_to_genename %>% distinct(uniprot_to_genename$uniprot_accession, .keep_all = TRUE) #same for the uniprot accession table
      uniprot_to_genename$better_gene_name = genes$SYMBOL #change to the gene name that AnnotationDbi gave 
      uniprot_to_genename[uniprot_to_genename$better_gene_name == uniprot_to_genename$gene_name & !is.na(uniprot_to_genename$better_gene_name), "better_gene_name"] = NA #if original gene name = better gene name, we make it NA to signal that we haven't changed this gene name
      #loop to change the gene names in the abundancy data table
      for(i in 1:nrow(abu_data)){
          name = abu_data$name[i] #take the orginal gene name in the abundancy table
          name = uniprot_to_genename[uniprot_to_genename$full_name == name, "gene_name"] #replace the full name with the gene name
          if(!is.na(uniprot_to_genename[uniprot_to_genename$gene_name == name, "better_gene_name"])){ #check if the gene name is NA in the 'better_gene_name" column, and if not:
            name = uniprot_to_genename[uniprot_to_genename$gene_name == name, "better_gene_name"] #replace the gene name with the "better gene name" from the AnnotationDbi
          }
          abu_data$name[i] = name #change the actual gene name in the abundancy data with the final selected gene name
      }
      abu_data$name = make.unique(abu_data$name) #make the gene names unique (required for Summarized Experiment)
      abu_data$ID = make.unique(abu_data$ID) #make the IDs unique (required for Summarized Experiment)
      nrow(abu_data) #number of proteins after removing double proteins according to the AnnotationDbi uniprot accession database

      #save the results of uniprot to genename annotation check in the results folder
      saveRDS(uniprot_to_genename, file = "results/uniprot_to_genename.rds")
...
#load new clinical dataset
      clin = read.csv(file = "data/Clinical_information_FINAL.csv") #load the clinical variables dataset
      clin = clin[, c("MAXOMOD.ID", #make a subselection of columns
                      "Tube.ID",
                      "CSF.proteomic..metabolomic..miRNA.ID", 
                      "Group", 
                      "Gender", 
                      "Age.at.collection", 
                      "NfL..pg.ml.", 
                      ...
                      "slow.vital.capacity", 
                      "pNFh")] 
      colnames(clin) = c("maxomod_id","tube_id","patid", "disease", "sex", "age", "neurofilaments","genetics", "onset", "age_at_onset", "progression_rate", "slow_vital_capacity", "pNFh") #rename the columns
      ...
      clin$neurofilaments = as.numeric(clin$neurofilaments) #the missing values will cause the warning
      clin$genetics = as.factor(clin$genetics)
      levels(clin$genetics) = c("C9orf72", "negative", "negative", "not_performed", "negative") #renaming the different values for negative to just "negative"
      clin$age_at_onset = as.numeric(clin$age_at_onset)
      clin$progression_rate = as.numeric(clin$progression_rate)
      clin$slow_vital_capacity = as.numeric(clin$slow_vital_capacity) #the missing values for the controls is causing a warning
      clin$pNFh = as.numeric(clin$pNFh) #the missing values for the controls is causing a warning
      ...
      clin$tube_id[!clin$tube_id %in% tube_id_abu_data] #ALS18 is not in there
      
      #add center variable
      clin$center = rep("munich", nrow(clin))
      clin$center[grep("Ctr", clin$maxomod_id)] = "goettingen" #the maxomod id's starting with "Ctr" are from goettingen
      ...
      clin = clin[tube_id_abu_data ,] #align clinical variables with proteomics data order, this also removes ALS18 because it is not in the abu data
      rownames(clin) = colnames(abu_data)[5:106] = clin$patid #now give both datasets the CSF patids again
```
...
#how to work with this data format can be found online when looking up the package 'SummarizedExperiment'
      ...
      abundance.columns <- grep("CSF", colnames(abu_data[,c("name","ID",patids_ALS)])) # get abundance column numbers
...
#combine male and female stratified results in one volcano plot
...
volcano_plot <- function(data_res, alpha_sig, name_title, labels){
  ...
  df <- data.frame(x = logFC, 
                   ...
                   name = data_res$name,
                   sex = data_res$sex)
  names(df) <- c("x","y","name", "sex")
  df <- df %>%
    mutate(omic_type = case_when(y >= (-log10(alpha_sig)) & sex == "female" ~ "sig_f",
                                 y >= (-log10(alpha_sig)) & sex == "male" ~ "sig_m",
                                 y < (-log10(alpha_sig)) & sex == "female" ~ "ns_f",
                                 y < (-log10(alpha_sig)) & sex == "male" ~ "ns_m")) 
  cols <- c("sig_f" = "#E78AC3", "sig_m" = "#66C2A5", "ns_f" = "#E78AC3", "ns_m" = "#66C2A5") 
  ggplot(data = df, aes(x,y)) + 
    geom_point(aes(colour = omic_type), 
               alpha = 0.3, 
               shape = 16,
    ...
    theme(axis.title.y = element_text(size = 14),
          ...
          plot.title = element_text(size = 15, hjust = 0.5),
          text = element_text(size = 14)) +
    annotate("text", x = 2, y = 0.5, label = paste0(sum(df$omic_type=="sig_f"), " significant in women \n", sum(df$omic_type=="sig_m"), " significant in men")) + 
    guides(colour = guide_legend(override.aes = list(alpha = c(0.3, 0.3, 0.7),
                                                     size = c(2, 2, 4))))
}

#first combine male and female results in one table
      f = res$imp_MinProb_age_cov_only_female
      f$sex = rep("female", nrow(f))
      
      m = res$imp_MinProb_age_cov_only_male
      m$sex = rep("male", nrow(m))
      
      results_male_female = rbind(f,m)
      
#make the plot
    plots = list()
    plots[[1]] = volcano_plot(results_male_female, 
                              0.05, 
                              "Volcano plot proteomics \nalpha = FDR 0.05\nsex stratified, age cov, imp MinProb",
                              labels = labels$imp_MinProb_age_cov_only_male)
    plots[[2]] = volcano_plot(results_male_female, 
                              0.1 , 
                              "Volcano plot proteomics \nalpha = FDR 0.1\nsex stratified, age cov, imp MinProb",
                              labels = labels$imp_MinProb_age_cov_only_male)
    
    ggarrange(plotlist = plots, ncol = 2, nrow = 1)
    ggsave("plots/paper/volcano_plots_sex_stratified_age_cov.pdf", 
...
#the files I load here are manually annotated files. Each pathway is categorized
ann_GSEA_results = list(
  all_patients = read_excel(path = "results/GSEA_clusterprofiler_imp_MinProb_age_sex_cov_all_patients_BP_LT_CM.xlsx"),
  only_female = read_excel(path = "results/GSEA_clusterprofiler_imp_MinProb_age_cov_only_female_BP_LT_CM.xlsx"),
  only_male = read_excel(path = "results/GSEA_clusterprofiler_imp_MinProb_age_cov_only_male_BP_LT_CM.xlsx")
)
...
#parameters for the analysis below
    ...
    empty_m = as.data.frame(matrix(nrow = 639, ncol = n_bs)) #generate empty matrix
