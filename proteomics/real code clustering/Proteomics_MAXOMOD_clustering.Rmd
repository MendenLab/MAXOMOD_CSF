---
title: "Proteomics MAXOMOD Clustering "
author: "Clara Meijs"
date: "2023-08-03"
output:
  html_document:
...
```{r load data}
      #we're loading a summarized experiment from the differential expression analysis
      se = readRDS(file = "data/summarized_experiment_for_clustering_imp_MinProb_ALS_ward.D2.rds")
      ...
      clin$onset = rep("bulbar", nrow(clin))
      clin$onset[grep("spinal", clin$ID)] = "spinal"
...
set.seed(9)
      ...
      cluster_assignments = as.data.frame(clin$label)
...
#settings for upcoming analyses
      cluster_assignments <- perform_clustering(assay, 9, clin_labels = clin$label) #use clustering function to get cluster labels
...
#make summarized experiment, this time with cluster as condition
        ...
        abundance.columns <- grep("CSF", colnames(assay2)) # get abundance column numbers
        experimental.design = clin[, c("label","cluster", "onset", "age", "sex", "neurofilaments", "genetics", "age_at_onset", "progression_rate", "age_cat")]
...
```{r Save results and plots for DEx and GSEA}
#save plots
...
allplots <- ggarrange(plotlist=plots,
                      ...
                      ncol = 2, nrow = 2)
ggsave("plots/fit_scores.pdf", width = 11, height = 8, units = "in") #CONCLUSION --> 3:5 Clusters is ideal
```
...
# set seed for reproducible results
      ...
      cluster_assignments <- perform_clustering(assay, 9, clin_labels = clin$label)
...
#load controls assay
SEs = readRDS("/Users/clara.meijs/Desktop/PhD/Proj_ALS_MAXOMOD/claras_code/proteomics/differential_expression_analysis/results/summarized_experiments_imputed.rds")
assay_all = as.data.frame(t(assay(SEs$imp_MinProb)))
rownames(assay_all) = SEs$imp_MinProb$label
...
```
## Visualization 6c: heatmap for validation cohort selection

```{r Visualization 6c: heatmap for validation cohort selection}

#WITH ONLY THE DIFFERENTIALLY ABUNDANT PROTEINS
      ...
      for(i in indices){
        set.seed(9)
           # Create row and column annotations for k2
                col_annotations <- data.frame(
                  Cluster_assignment_k2 = as.character(heatmap_data$patient_cluster[[i]])
                )
                rownames(col_annotations) = heatmap_data$patids[[i]]
                ...
                save_pheatmap_pdf(p, paste0("plots/heatmap_clustering_kmeans_",title,"_validation_selection.pdf"))
        }
...
# WITH ALL PROTEINS
        ...
        set.seed(9)
           ...
           # Create row and column annotations for k2
                col_annotations <- data.frame(
                  Cluster_assignment_k2 = cluster_assignments_2$`kmeans_k=2`,
                  Cluster_assignment_k3 = cluster_assignments_2$`kmeans_k=3`)
                
                rownames(col_annotations) = cluster_assignments$patid
                
                ann_colors = list(
                  Cluster_assignment_k2 = final_colours$clustering,
                  Cluster_assignment_k3 = final_colours$clustering)
                ...
                p = pheatmap(
                  ...
                  fontsize = 5,  # Adjust the font size if needed
                  border_color = NA, 
                  main = "all_proteins_hierarchical_clustering"
                )
                save_pheatmap_pdf(p, paste0("plots/heatmap_clustering_kmeans_validation_selection_all_proteins.pdf"))
...
excel_file <- "results/GSEA_results_kmeans_LT_CM.xlsx"

# Get the names of all sheets in the Excel file
sheet_names <- excel_sheets(excel_file)

# Read each sheet into a list
GSEA_results <- lapply(sheet_names, function(sheet) {
  read_excel(excel_file, sheet = sheet, skip = 0)
})

names(GSEA_results) = sheet_names
GSEA_results = GSEA_results[1:3]

for(i in 1:length(GSEA_results)){
  
      GSEA_results[[i]]$Laura_annotation = as.factor(GSEA_results[[i]]$Laura_annotation)
      GSEA_results[[i]]$ID = as.character(GSEA_results[[i]]$ID)
      GSEA_results[[i]]$log10 = as.numeric(GSEA_results[[i]]$log10)
      ...
      GSEA_results[[i]]$ID <- sapply(GSEA_results[[i]]$ID, function(text) {
        limit_words(text, 7)
      })
      
      # Create a custom color palette with 7 distinct colors
      colour_subset <- colour_set[1:length(levels(GSEA_results[[i]]$Laura_annotation))]
      
      GSEA_results[[i]]$ID = as.factor(GSEA_results[[i]]$ID)
      desired_order = rev(unique(GSEA_results[[i]]$ID))
      # Set the category variable as an ordered factor with desired order
      GSEA_results[[i]]$ID <- factor(GSEA_results[[i]]$ID, levels = desired_order, ordered = TRUE)
      
      # Create a bar plot with ggplot2
      ggplot(GSEA_results[[i]], aes(x = ID, y = log10, fill = Laura_annotation)) +
        geom_bar(stat = "identity", position = "identity") +
      ...
      ggsave(paste0("plots/barplot_Lauras_annotation_", names(GSEA_results)[i],"version1.pdf"),
             width = 8, 
             height = 1 + nrow(GSEA_results[[i]])*0.1,
             units = "in")
...
levels(GSEA_results$theta_vs_beta_k3$Laura_annotation)["cel growth / morphogenesis /development"] = "cell_growth/morphogen"
levels(GSEA_results$theta_vs_alpha_k3$Laura_annotation)["cel growth / morphogenesis /development"] = "cell_growth/morphogen"

for(i in 1:length(GSEA_results)){
  order = as.character(unique(GSEA_results[[i]]$Laura_annotation))
  GSEA_results[[i]]$Laura_annotation = factor(GSEA_results[[i]]$Laura_annotation, levels = order)
  
  comparison = names(GSEA_results)[i]
  comparison = str_split(comparison, pattern = "_")[[1]]
  up = comparison[1]
  down = comparison[3]
  GSEA_results[[i]]$comparison = rep(up, nrow(GSEA_results[[i]]))
  GSEA_results[[i]]$comparison[GSEA_results[[i]]$enrichmentScore < 0] = down


# Create a bar plot with ggplot2

  #WHITE FACETGRID
      ggplot(GSEA_results[[i]], aes(x = ID, y = log10, fill = comparison)) +
        ...
        facet_grid(Laura_annotation ~ ., scales = "free", space = "free") +
        theme(strip.text.y = element_text(angle = 0)) + 
        theme(panel.spacing = unit(0.1, "lines"))
            
      ggsave(paste0("plots/paper/barplot_Lauras_annotation_", names(GSEA_results)[i],"_facetgrid_white.pdf"),
             width = 5, 
             height = 1 + nrow(GSEA_results[[i]])*0.06,
             units = "in")
  ...
  #BLACK FACETGRID    
      ...
      ggsave(paste0("plots/paper/barplot_Lauras_annotation_", names(GSEA_results)[i],"_facetgrid_black.pdf"),
             width = 5, 
             height = 1 + nrow(GSEA_results[[i]])*0.06,
             units = "in")
    ...
    #NO FACETGRID   
      ...
      ggsave(paste0("plots/paper/barplot_Lauras_annotation_", names(GSEA_results)[i],".pdf"),
             width = 4, 
             height = 1 + nrow(GSEA_results[[i]])*0.04,
             units = "in")
}
```
...
#make summarized experiment, this time with cluster as condition, MALE
        ...
        assay_male = assay[clin$label[clin$sex == "Male"],]
        levels(clin_male$cluster) = cluster_levels[1:length(levels(clin_male$cluster))]
        assay2 = as.data.frame(t(assay_male))
        assay2$ID = assay2$name = rownames(assay2)
        abundance.columns <- grep("CSF", colnames(assay2)) # get abundance column numbers
        experimental.design = clin_male[, c("label","cluster", "onset", "age", "sex", "neurofilaments", "genetics", "age_at_onset", "progression_rate", "age_cat")]
        colnames(experimental.design) = c("label","condition", "onset", "age", "sex", "neurofilaments", "genetics", "age_at_onset", "progression_rate", "age_cat")
        experimental.design$replicate = 1:nrow(experimental.design)
        ...
        assay_female = assay[clin$label[clin$sex == "Female"],]
        levels(clin_female$cluster) = cluster_levels[1:length(levels(clin_female$cluster))]
        assay2 = as.data.frame(t(assay_female))
        assay2$ID = assay2$name = rownames(assay2)
        abundance.columns <- grep("CSF", colnames(assay2)) # get abundance column numbers
        experimental.design = clin_female[, c("label","cluster", "onset", "age", "sex", "neurofilaments", "genetics", "age_at_onset", "progression_rate", "age_cat")]
        colnames(experimental.design) = c("label","condition", "onset", "age", "sex", "neurofilaments", "genetics", "age_at_onset", "progression_rate", "age_cat")
        experimental.design$replicate = 1:nrow(experimental.design)
...
results = list()
for(i in 1:10){
  results[[i]] <- perform_clustering(assay, i, clin_labels = clin$label)
}
...
for(i in 1:10){
  ...
  results[[i]] <- perform_clustering(data, seed = 9, clin_labels = clin_data$label)
}
...
#number of genes reported in the Tam et al paper:
nrow(Tam_genes)

#number of genes overlap between our dataset and those in the Tam et al paper:
sum(Tam_genes$Gene %in% colnames(expression_matrix))
...
set.seed(9)
      ...
      cluster_assignments_age_adj = as.data.frame(clin$label)
